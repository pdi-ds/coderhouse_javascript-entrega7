...
const fail = 'fail';
const div = 'div';
const locData = { name: '', from: 'ARS', amount: 100, to: [] };
const strDTS = {};
(function () {
	const str = { ...locData };
...
const call = {
	'sN1': function () {
		let next = this.querySelector('button[data-role="next"]'), input = this.querySelector('input[name="name"]');
		input.value = strDTS.name;
		eventList(next, 'click', e => {
			let name = input.value;
			if (name === null || name === '') { validate({ input, message: 'Ingresá tu nombre' }); }
			else { strDTS.name = name; clear(input); usrStep('sN2'); }
		});
	},
	'sN2': function () {

		let select = this.querySelector('select'), input = this.querySelector('input[name="amount"]'), prev = this.querySelector('button[data-role="previous"]'), next = this.querySelector('button[data-role="next"]'), currencies = [];
		...
		eventList(prev, 'click', e => { usrStep('sN1'); });
		eventList(next, 'click', e => {
			let from = select.value, amount = input.value, passed = [], reset = false;
	...
	'sN3': function ({ reset } = { reset: false }) {
		function reg(selected = false) {
			...
			currencies = currency.curGET(strDTS.from).map(function (type) {
				let option = document.createElement('option');
				option.value = type
				option.textContent = currency[type].id; (selected !== false && type === selected) && (option.selected = true); return option;
			});
		...
		eventList(prev, 'click', e => { usrStep('sN2'); });
		eventList(next, 'click', e => {
	...
	'sN4': function () {
		...
		eventList(prev, 'click', e => { usrStep('sN3'); });
		eventList(restart, 'click', e => {
		...
		conversions = strDTS.to.map(value => {
			let { from, amount } = strDTS, converted = currency.curCONV(from, value, amount);
...
function clear(input) {
	let alert = input.parentNode.querySelector(`${div}.${fail}`);
	input.className = '';
	alert !== null && alert.remove();
}
function eventList(element, event, callcback) {
	let attribute = 'data-attached', attached = element.getAttribute(attribute) !== null;
	if (attached === false) { element.setAttribute(attribute, '1'); element.addEventListener(event, callcback); }
}
function usrStep(name, args = []) {

	let container = document.querySelector('.container'), step = container.querySelector(`[data-step="${name}"]`), callback;
	if (step !== null) {
		container.querySelectorAll('[data-step]').forEach(current => { current.className = ''; });
		callback = step.getAttribute('data-callback');
		(callback !== null && call[callback]) && call[callback].call(step, ...args);
